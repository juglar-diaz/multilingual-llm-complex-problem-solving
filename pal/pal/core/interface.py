...
class TextInterface:
    ...
    def __init__(
        ...
        self.history = []
        self.answer_prefix = answer_prefix[lang]
    ...
    def run(self, prompt, temperature=0.0, top_p=1.0, majority_at=None, max_tokens=512):
        gen = call_gpt(prompt, model=self.model, stop=self.stop, 
            temperature=temperature, top_p=top_p, max_tokens=max_tokens, majority_at=majority_at, lang=self.lang)
...
class ProgramInterface:
    ...
    def generate(self, prompt: str, temperature: float =0.0, top_p: float =1.0, max_tokens: int =512, majority_at: int =None):
        gens = call_gpt(
            prompt, 
            model=self.model, 
            stop=self.stop, 
